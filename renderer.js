...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function getCompatibleFilaments() {
        ...
        if (selPrint.value && !selPrint.value.includes('Выберите')) {
    ...
    function calculateMaxObjects() {
        ...
        try {
            // Для Orca Slicer используем специальную логику
            if (currentSlicer === 'orca') {
                const orca = new OrcaIntegration();
                const config = orca.getConfigForPATest(selectedPrinter.name, selFilament.value);
                if (!config) return null;

                const bedWidth = config.printer.bed_size_x || 200;
                const bedHeight = config.printer.bed_size_y || 200;
                const nozzleDiameter = config.printer.nozzle_diameter || 0.4;
    ...
    function updateVisualizationOrWarnings(issues) {
        ...
        if (issues.length > 0) {
            // Добавляем предупреждение над визуализацией
      ...
      `;
        ...
        // Очищаем старое предупреждение рядом с G-code
        document.getElementById('warningGenerate').innerHTML = '';
    }
    ...
    function calculateGCodePlaceholders() {
        try {
            ...
            if (currentSlicer === 'orca') {
                ...
                return {
                    'nozzle_diameter': config.printer.nozzle_diameter.toFixed(3),
                    ...
                    'flow_ratio': config.filament.flow_ratio.toString(),
            ...
            return {
                ...
                'filament_flow_ratio': getConfigValue('filament_flow_ratio', getConfigValue('extrusion_multiplier', '1')),
    ...
    function generateFilename() {
        try {
            // Для Orca Slicer используем простое имя
            if (currentSlicer === 'orca') {
                let partsPrinterName = selectedPrinter.name.split('*');
                let printerName = partsPrinterName.length > 1 ? partsPrinterName[1] : selectedPrinter.name;
            ...
            console.error('Ошибка генерации имени файла:', e);
    ...
    async function sendToPrinter() {
        ...
        try {
            ...
            let printerHost;
            if (selectedPrinter.type === 'orca') {
                printerHost = selectedPrinter.print_host;
            } else {
                const physicalConfig = parseIniFile(physicalConfigPath);
                printerHost = physicalConfig.print_host;
            }
    ...
    function generateGCode() {
        ...
        try {
            ...
            // Проверяем G-code на проблемы и обновляем визуализацию
    ...
    function saveSettings() {
        try {
            ...
            if (selectedPrinter && selPrinter.value) {
                ...
                settings.slicers[currentSlicer].printers[selPrinter.value] = {
    ...
    function loadOrcaPrinters() {
        try {
            const orca = new OrcaIntegration();
            // Устанавливаем путь для Orca Slicer
            currentSlicerPath = orca.parser.orcaPath;
            ...
            if (physicalPrinters.length > 0) {
                options += '<option disabled>--- Физические принтеры ---</option>';
                physicalPrinters.forEach(p => {
                    options += `<option value="${p.name}">${p.name} [${p.print_host}]</option>`;
                });
            }


            selPrinter.innerHTML = options;
            selPrinter.disabled = printers.length === 0;

            if (printers.length === 0) {
                selPrinter.innerHTML = '<option>OrcaSlicer не найден</option>';
            }
        } catch (e) {
            console.error('Ошибка загрузки принтеров Orca:', e);
            selPrinter.innerHTML = '<option>Ошибка загрузки OrcaSlicer</option>';
            selPrinter.disabled = true;
        }
    }

    function loadOrcaFilaments() {
        try {
            const orca = new OrcaIntegration();
            const filaments = orca.getAvailableFilaments(selectedPrinter?.name);

            let options = '<option value="">Выберите филамент</option>';
            filaments.forEach(f => {
                options += `<option value="${f.id}">${f.name}</option>`;
            });

            selFilament.innerHTML = options;
            selFilament.disabled = filaments.length === 0;
        } catch (e) {
            console.error('Ошибка загрузки филаментов Orca:', e);
            selFilament.innerHTML = '<option>Ошибка загрузки</option>';
            selFilament.disabled = true;
        }
    }

    function loadOrcaProcesses() {
        try {
            const orca = new OrcaIntegration();
            const processes = orca.getAvailableProcesses(selectedPrinter?.name);

            let options = '<option value="">Выберите профиль печати</option>';
            processes.forEach(p => {
                options += `<option value="${p.name}">${p.name}</option>`;
            });

            selPrint.innerHTML = options;
            selPrint.disabled = processes.length === 0;
        } catch (e) {
            console.error('Ошибка загрузки процессов Orca:', e);
            selPrint.innerHTML = '<option>Ошибка загрузки</option>';
            selPrint.disabled = true;
        }
    }
